...
export async function generateStaticParams() {
  const static151Pokemon = Array.from({ length: 151 }).map((_, index) => `${index + 1}`);
...
const getPokemon = async (id: string): Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`, { cache: 'force-cache' }).then((response) =>
      response.json()
    );
