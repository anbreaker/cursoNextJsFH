...
export async function generateStaticParams() {
  const data: PokemonsReponse = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=151`).then((response) =>
    response.json()
  );

  const static151PokemonNames = data.results.map((pokemon) => ({
    name: pokemon.name,
  }));

  return static151PokemonNames.map(({ name }) => ({ name }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    return {
      title: `${pokemonName} - Number ${id}`,
      description: `Pokemon Page ${pokemonName} description`,
    };
  } catch (error) {
    return {
      title: 'Pokémon Page not found',
...
const getPokemon = async (name: string): Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, { cache: 'force-cache' }).then(
